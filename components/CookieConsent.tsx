...
function CookieToggle({ checked, disabled = false, onChange }: { checked: boolean; disabled?: boolean; onChange?: () => void }) {
  return (
    <button
      ...
      className={`relative h-7 w-14 rounded-full transition ${checked ? 'bg-green-600' : 'bg-gray-300'} ${disabled ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}
    >
      <span className={`absolute top-1 h-5 w-5 rounded-full bg-white shadow transition ${checked ? 'left-8' : 'left-1'}`} />
    </button>
  );
}
            ...
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl">
                ...
                <p className="font-montserrat text-sm leading-7 text-gray-600">
                  ...
                  <button type="button" onClick={() => setShowSettings(true)} className="font-semibold text-green-700 underline underline-offset-4">Nastavenia</button>{' '}
                  si môžete upraviť svoje preferencie. Viac informácií nájdete na stránke{' '}
                  <Link href="/ochrana-osobnych-udajov" className="font-semibold text-green-700 underline underline-offset-4">Ochrana osobných údajov</Link>.
                </p>
              </div>
              <div className="flex flex-col gap-3 sm:flex-row">
                <button type="button" onClick={rejectOptional} className="rounded-xl border border-gray-300 px-5 py-3 font-jakarta text-sm font-bold text-gray-700 transition hover:border-gray-400 hover:bg-gray-50">Odmietnuť voliteľné</button>
                <button type="button" onClick={acceptAll} className="rounded-xl bg-green-600 px-5 py-3 font-jakarta text-sm font-bold text-white transition hover:bg-green-700">Prijať všetko</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-2xl overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-5">
              ...
              <button type="button" onClick={() => setShowSettings(false)} className="rounded-full border border-gray-200 px-3 py-1 text-2xl leading-none text-gray-500 transition hover:border-gray-300 hover:text-gray-900">×</button>
            </div>
            ...
            <div className="flex flex-col gap-3 border-t border-gray-200 px-6 py-5 sm:flex-row sm:justify-end">
              <button type="button" onClick={rejectOptional} className="rounded-xl border border-gray-300 px-5 py-3 font-jakarta text-sm font-bold text-gray-700 transition hover:border-gray-400 hover:bg-gray-50">Odmietnuť voliteľné</button>
              <button type="button" onClick={saveCurrentSettings} className="rounded-xl border border-green-200 bg-green-50 px-5 py-3 font-jakarta text-sm font-bold text-green-700 transition hover:bg-green-100">Uložiť nastavenia</button>
              <button type="button" onClick={acceptAll} className="rounded-xl bg-green-600 px-5 py-3 font-jakarta text-sm font-bold text-white transition hover:bg-green-700">Prijať všetko</button>
            </div>
          </div>
        </div>
