...
export default function Header() {
  ...
  return (
    <header className="bg-gray-950 text-white fixed w-full top-0 z-50 border-b border-gray-800">
      <div className="container mx-auto px-4 flex justify-between items-center h-20">
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <Link href="/kontakt" className="border border-yellow-400 text-yellow-400 hover:bg-yellow-400 hover:text-black font-bold font-jakarta text-sm px-5 py-2 rounded transition-colors">
            KONTAKT
          </Link>
        </div>
      ...
      {/* Mobile Overlay */}
      <div className={`fixed top-0 left-0 w-full h-[100dvh] bg-gray-950 z-[60] transform transition-transform duration-300 ease-in-out md:hidden ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex flex-col p-8 gap-6">
          ...
          <a href="tel:+421908071885" className="text-yellow-400 font-bold font-montserrat text-xl mt-4" onClick={() => setIsMenuOpen(false)}>+421 908 071 885</a>
        </nav>
      </div>
    </header>
  );
