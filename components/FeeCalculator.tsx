    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {BOXES.map(({ type, label }) => (
          <button
            ...
            className="relative h-20 text-left px-4 cursor-pointer transition-all duration-300 bg-transparent border border-white/10 hover:scale-[1.02]"
            style={
              active === type
                ? {
                    boxShadow: `0 0 0 2.5px ${ACCENT}, 0 8px 20px rgba(22,163,74,0.3)`,
        ...
        <div className="mt-8 space-y-6">
          <div>
            ...
            <select
              ...
              className="w-full max-w-sm border border-white/10 bg-transparent text-white px-3 py-2.5 text-sm font-montserrat focus:outline-none appearance-none cursor-pointer"
      ...
      {price !== null && (
        <div className="mt-8 font-jakarta text-xl text-white font-bold">
          ...
          <span className="text-3xl" style={{ color: ACCENT }}>{price} €</span>
        </div>
      )}
    </div>
  );
