...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-950">
        ...
        <div className="container mx-auto px-4 pt-28 pb-20">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/ponuka" className="bg-green-600 hover:bg-green-500 px-8 py-4 rounded text-white font-bold text-lg font-jakarta border border-green-400 transition-colors text-center">
                PONUKA
              </Link>
              <Link href="/kontakt" className="bg-gray-800 hover:bg-gray-700 border border-gray-600 px-8 py-4 rounded text-white font-bold text-lg font-jakarta transition-colors text-center">
                Kontakt
              </Link>
            </div>
          </div>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              {featured.map((car) => {
                ...
                return (
                  <Link key={car.id} href={`/vozidlo/${car.slug}`} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden hover:border-green-600 hover:-translate-y-1 transition-all duration-200 flex flex-col">
                    <div className="relative aspect-[4/3] bg-gray-700 overflow-hidden">
                      ...
                      {car.isReserved && <div className="absolute top-2 left-2 bg-gray-900 text-white text-xs font-bold font-montserrat px-2 py-1 rounded">REZERVOVANÉ</div>}
                    </div>
                    <div className="p-3 flex flex-col flex-1">
                      ...
                      <div className="grid grid-cols-2 gap-1 mb-3 flex-1">
                        {car.year > 0 && <div className="flex items-center gap-1 text-xs font-montserrat text-gray-400"><img src="/icons/rok.svg" alt="" className="w-3.5 h-3.5 opacity-70" /><span>{car.year}</span></div>}
                        {car.mileage > 0 && <div className="flex items-center gap-1 text-xs font-montserrat text-gray-400"><img src="/icons/kilometre.svg" alt="" className="w-3.5 h-3.5 opacity-70" /><span>{car.mileage.toLocaleString('sk-SK')} km</span></div>}
                        {car.fuel && <div className="flex items-center gap-1 text-xs font-montserrat text-gray-400"><img src="/icons/palivo.svg" alt="" className="w-3.5 h-3.5 opacity-70" /><span>{car.fuel}</span></div>}
                        {car.transmission && <div className="flex items-center gap-1 text-xs font-montserrat text-gray-400"><img src="/icons/prevodovka.svg" alt="" className="w-3.5 h-3.5 opacity-70" /><span className="truncate">{car.transmission}</span></div>}
                      </div>
                      <div className="border-t border-gray-700 pt-2">
                        <p className="text-xl font-bold font-jakarta text-green-400">{car.price > 0 ? `${car.price.toLocaleString('sk-SK')} €` : 'Cena na vyžiadanie'}</p>
                      </div>
                    </div>
                  </Link>
                );
            ...
            <div className="text-center mt-10">
              <Link href="/ponuka" className="inline-block bg-green-600 hover:bg-green-500 text-white font-bold font-jakarta px-10 py-4 rounded-lg text-lg transition-colors">
                Všetky autá →
              </Link>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-start md:gap-16 mb-10">
              <div className="flex-1 mb-6 md:mb-0">
                <p className="text-green-400 font-montserrat font-light text-xs uppercase tracking-widest mb-2">JAZDENÉ</p>
      ...
      <section className="py-16 bg-gray-900">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
            {SERVICES.map((s) => (
              <div key={s.title} className="bg-gray-800 rounded-xl p-6 border border-gray-700 hover:border-green-600 transition-colors">
      ...
      <section className="h-96 bg-gray-800">
        <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d2635.0!2d19.7005!3d48.8067!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x0%3A0x0!2zRnXEjcOra292YSAxMTQ1LzI0LCBCcmV6bm8!5e0!3m2!1ssk!2ssk!4v1600000000000" width="100%" height="100%" style={{ border: 0 }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Jazdené mapa" />
      </section>
    </div>
  );
}
